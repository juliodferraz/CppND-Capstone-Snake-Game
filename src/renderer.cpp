...
void Renderer::Render(const World& world) {
  ...
  const std::deque<SDL_Point>& snake_position = snake.GetBodyPosition();
  ...
  for (SDL_Point const &point : snake_position) {
  ...
  // Render snake's head
  block.x = snake_position.front().x * block.w;
  block.y = snake_position.front().y * block.h;
  if (snake.IsAlive()) {
